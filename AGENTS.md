...
- **待办**：AI 精读队列已清空——2332 条全部有精读载荷（aiDone = 2332 / 2332，unread = 0，最新版本 20260925q，18 个本地提交**未推送**）。2026-09-25 这一轮用子 agent 并行新增 1080 篇，逐条复验 **0 条不达标**。真正的遗留项是会话之前就在的 486 条旧精读达不到新门槛（na 偏少 204、阶段覆盖 <50% 124、缺 p 214、含反引号 38、n 偏短 11、t 偏短 7）：它们仍通过仓库自带 ai-audit.mjs 的旧门槛，不是坏数据，只是比新标准薄，补齐属独立工作量。另 vid 777536 不是抓取缺口而是子 agent 凭空产出的 out/ 文件——它既不在 2332 条索引内、也没有 graph 文件，已被 commit-ai.cjs 以「图文件不存在」拒收，未污染任何数据。教训：回报里出现的 vid 不能直接采信，要以索引为准。其余遗留：graph 节点 params 逐条增强（现靠 widget-help.js 推导）；详情页 390px 横向溢出未修（原版就有，见 [docs/themes.md](../docs/themes.md) 末节）。注意 mark.cjs 会把本行覆写成通用模板，跑完需手工补回。
- **精读落盘工具链**（2026-09-25 起用，脚本在仓库外 `~/.cache/comfyui-jingdu/`，不入库）：`dump2.cjs` 取真实图上下文（比 `ai-audit.mjs dump` 多出端口名连线、拓扑 heads/tails/isolated、作者完整原文），`commit-ai.cjs` 按已验收样本的门槛批量校验并回写 graph + 索引。门槛（按图规模自适应）：`s` ≥220 字、`u` ≥3 项且每项 ≥15 字、`d` ∈1~3、`st` ≥min(3, 功能节点数) 且功能节点覆盖 ≥50%、`na` ≥max(min(4,功能节点数), min(20, ceil(功能节点×0.3)))，`st`/`na` 引用的节点 ID 必须在图里真实存在；卫生检查只作用于字符串值（反引号 / markdown / 制表符 / ASCII 单引号 / TODO 类占位词）——务必别对 `JSON.stringify` 结果做卫生检查，结构性的 `}}` 会误报。`_sources/` 那套构建脚本在本机不存在，索引与 graph 只能走 `commit-ai.cjs`。
