...
- **待办**：AI 精读队列已清空——2332 条全部有精读载荷（aiDone = 2332 / 2332，unread = 0，最新版本 20260925q，**18 个本地提交未推送**）。本轮（2026-09-25）用子 agent 并行新增 1080 篇，逐条复验 **0 条不达标**。真正的遗留项：会话之前就已存在的 486 条旧精读达不到新门槛（na 注释偏少 204、阶段覆盖 <50% 124、缺 p 214、含反引号 38、n 偏短 11、t 偏短 7）——它们仍通过仓库自带的 ai-audit.mjs 旧门槛，不是坏数据，只是比新标准薄；若要补齐需按 vid 重跑一轮，属独立工作量。另有 1 条（vid 777536）在 civitai 索引里但 graph 文件从未抓到，无法精读，属抓取缺口不是精读缺口。其余遗留：工作流 graph 节点 params 逐条增强（当前靠 widget-help.js 自动推导）；详情页 390px 横向溢出未修（原版就有，见 [docs/themes.md](../docs/themes.md) 末节）。
