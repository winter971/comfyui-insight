...
- **待办**：AI 精读队列已清空——2332 条全部有精读载荷（aiDone = 2332 / 2332，unread = 0，最新版本 20260925q，18 个本地提交**未推送**）。2026-09-25 这一轮用子 agent 并行新增 1080 篇，逐条复验 **0 条不达标**。真正的遗留项是会话之前就在的 486 条旧精读达不到新门槛（na 偏少 204、阶段覆盖 <50% 124、缺 p 214、含反引号 38、n 偏短 11、t 偏短 7）：它们仍通过仓库自带 ai-audit.mjs 的旧门槛，不是坏数据，只是比新标准薄，补齐属独立工作量。另 vid 777536 在 civitai 索引内但 graph 从未抓到，属抓取缺口、无法精读。其余遗留：graph 节点 params 逐条增强（现靠 widget-help.js 推导）；详情页 390px 横向溢出未修（原版就有，见 [docs/themes.md](../docs/themes.md) 末节）。注意 mark.cjs 会把本行覆写成通用模板，跑完需手工补回。
